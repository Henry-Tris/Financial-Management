...
public class ServiceReport {
	...
	public Map<YearMonth, Double> monthlyExpenses() {
		...
		for (Deal deal : cashFlow.getDeals()) {
			if (deal.getType() == TypeStatus.DESPESA) {
	...
	public double percentageIncrease(YearMonth previousMonth, YearMonth currentMonth) {
	    ...
	    double previousTotal = expenses.getOrDefault(previousMonth, 0.0);
	    double currentTotal = expenses.getOrDefault(currentMonth, 0.0);
	...
	public String checkAlert(YearMonth previousMonth, YearMonth currentMonth, double thresholdPercentage) {
	    ...
	    if (increase > thresholdPercentage) {
	        return "Alerta: seus gastos aumentaram " + String.format("%.2f", increase) + "% em relação ao mês anterior.";
	    }
	...
	public double availableToSpend() {
	    double balance = cashFlow.currentBalance();
	    double income = cashFlow.expectedIncome();
	    double expenses = cashFlow.expectedExpenses();
	...
	public String generateSummary() {
	    ...
	    summary.append("Previsão de entrada: R$ ").append(String.format("%.2f", cashFlow.expectedIncome())).append("\n");
	    summary.append("Previsão de saída: R$ ").append(String.format("%.2f", cashFlow.expectedExpenses())).append("\n");
	    ...
	    if (mostExpensive != null) {
	        summary.append("Categoria que mais consome: ").append(mostExpensive.getName()).append("\n");
	    }
	...
	public String budgetSummary() {
	    ...
	    double poupanca = expenses.getOrDefault(BudgetBucket.POUPANCA, 0.0);

	    double recommendedNecessidades = income * 0.50;
	    double recommendedDesejos = income * 0.30;
	    double recommendedPoupanca = income * 0.20;

	    StringBuilder summary = new StringBuilder();
	    summary.append("=== Método 50/30/20 ===\n");
	    summary.append("Necessidades: R$ ").append(String.format("%.2f", necessidades))
	           .append(" (recomendado: R$ ").append(String.format("%.2f", recommendedNecessidades)).append(")\n");
	    summary.append("Desejos: R$ ").append(String.format("%.2f", desejos))
	           .append(" (recomendado: R$ ").append(String.format("%.2f", recommendedDesejos)).append(")\n");
	    summary.append("Poupança: R$ ").append(String.format("%.2f", poupanca))
	           .append(" (recomendado: R$ ").append(String.format("%.2f", recommendedPoupanca)).append(")\n");
